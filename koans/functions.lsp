...
(define-test test-lexical-closure-interactions
  ...
  (let ((tangled-funs-1 (two-funs 1))
    ...
    (flet ((get-x (obj) (funcall (first obj)))
           (set-x (obj x) (funcall (second obj) x)))
     (assert-equal (get-x tangled-funs-1) 1)
     (set-x tangled-funs-1 0)
     (assert-equal (get-x tangled-funs-1) 0)

     (assert-equal (get-x tangled-funs-2) 2)
     (set-x tangled-funs-2 100)
     (assert-equal (get-x tangled-funs-2) 100))))
